...
- **`src/App.svelte`** — the **only** place that watches state and drives audio. Three `$effect`s: load dry sound on `drySource` change; rebuild IR on `irKey` change; re-render output on dry/ir/`byHand` change. Each uses a `stale` flag to discard out-of-order async results. Also owns layout — including the **mutually-exclusive Time/Frequency domain toggle** (only one display is mounted at a time, filling the full height) — and keyboard shortcuts.
...
- **`src/components/Waveform.svelte` / `Spectrum.svelte`** — the two display components share one layout discipline: **dry, IR and output as three stacked lanes** (never overlaid), each lane labelled in its own trace color on the right. Waveform draws the time domain on a shared, zoom/pan-able seconds axis; Spectrum draws the frequency domain on a shared linear 20 Hz–20 kHz axis, with a **Buffers** source (static `magnitudeDb` per buffer, cached per buffer identity) and a **Live** source (only the playing lane animates from the `AnalyserNode`; the others stay static and dimmed). Only one of the two is mounted at a time (see the App.svelte toggle).
